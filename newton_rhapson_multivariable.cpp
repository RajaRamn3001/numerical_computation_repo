 ...
 {for(int j=0;j<J.size();j++)
  {
   ...
   {
   case 0:
   J[i][j]=(f1(x[0]+dx[j],x[1],x[2],i,&k)-f1(x[0]-dx[j],x[1],x[2],i,&k))/(2*dx[j]);
   *func_eval=*func_eval+3;
    break;
   case 1:
   J[i][j]=(f1(x[0],x[1]+dx[j],x[2],i,&k)-f1(x[0],x[1]-dx[j],x[2],i,&k))/(2*dx[j]);
   *func_eval=*func_eval+3;
    break;
   case 2:
   J[i][j]=(f1(x[0],x[1],x[2]+dx[j],i,&k)-f1(x[0],x[1],x[2]-dx[j],i,&k))/(2*dx[j]);
   *func_eval=*func_eval+3;
    break;
